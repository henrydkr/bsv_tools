...
class SendNote(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def sendnote():
            ...
            list_of_pushdata = [var.encode('utf-8'), var2.encode('utf-8')]
        ...
        def send_meta_note():
            ...
            var=note_widget.get()
            meta="meta"
            list_of_pushdata = [meta.encode('utf-8'),var.encode('utf-8')]
            bob=my_key.send_op_return(list_of_pushdata)
            note_tx.configure(text=bob)
        ...
        Send_Meta_Note = tk.Button(self, text='Send Meta Note',  # when click on this button, call the show_frame method to make SendNote appear
                            command=send_meta_note)
        Send_Meta_Note.grid(row=6, column=8)
